...
def getWeatherForecast():
    ...
    df['preciptype'] = df['preciptype'].astype(pd.StringDtype())
    df['stations'] = df['stations'].astype(pd.StringDtype())
    df['preciptype'] = df['preciptype'].str.replace('[', '', regex=True)
    df['preciptype'] = df['preciptype'].str.replace(']', '', regex=True)
    df['preciptype'] = df['preciptype'].str.replace('\'', '', regex=True)
    df['preciptype'] = df['preciptype'].str.replace(' ', '', regex=True)
    df['stations'] = df['stations'].str.replace('[', '', regex=True)
    df['stations'] = df['stations'].str.replace(']', '', regex=True)
    df['stations'] = df['stations'].str.replace('\'', '', regex=True)
